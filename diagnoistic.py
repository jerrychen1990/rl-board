...
from rlb.board_core import Episode, BoardState
from rlb.engine import run_board_games
from rlb.evaluate import evaluate_episodes
from rlb.gobang import GravityGoBang33, TicTacToe
...
def explore_self_play(ckpt, episodes, level=logging.DEBUG):
    ...
    with LoggingLevelContext(level=level, loggers=[logging.root, logging.getLogger("rlb.mcst")]):
        history, scoreboard = run_board_games(agents=agents, episodes=episodes, env=env, mode="train",
                                              shuffle=True, is_render=True, callbacks=[],
                                              show_episode_size=1)
...
def explore_records():
    eval_rs_list = []
    minmax_agent.train(env_cls=env_cls, value_decay=0.95, overwrite_cache=False)
    perfect_ac_infos = minmax_agent.gen_ac_infos()
...
def explore_minmax():
    minmax_agent.train(env_cls=env_cls, value_decay=1., overwrite_cache=True)
    node = minmax_agent.root
    while True:
        logger.info(f"{BoardState.from_obs(node.state)}, value={node.value}")
...
if __name__ == "__main__":
    ...
    # explore self play
    # history, scoreboard = explore_self_play(ckpt=True, episodes=1, level=logging.INFO)
